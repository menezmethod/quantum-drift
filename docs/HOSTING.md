...
Run npm test, npm run build, and CHROME_BACKEND=native npm run test:browser before release. Deploy an explicit tested commit. Wait for healthy HTTP and check two real remote clients sharing a room. Roll back to the preceding tested commit through Coolify; keep the rankings volume. Deployments currently end active matches; schedule updates between play sessions. GitHub release downloads include a prebuilt browser client and the Node server, plus checksums.
